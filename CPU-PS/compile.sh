...
nvcc -O3 -std=c++11 -Xcompiler -fPIC -gencode=arch=compute_20,code=sm_20 -gencode=arch=compute_30,code=sm_30 -gencode=arch=compute_35,code=sm_35 -gencode=arch=compute_52,code=sm_52 -gencode=arch=compute_61,code=sm_61\
 -I/opt/openmpi-2.0.2-ca/include -L/opt/openmpi-2.0.2-ca/lib -lmpi_cxx -lmpi prog.cpp -o prog.exe
