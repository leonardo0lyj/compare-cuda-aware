...
int main(int argc, char** argv) 
{
	...
	std::cout << "CPU GatherBcast Start." << std::endl;
	...
	MPI::COMM_WORLD.Gather(&grad_array[0], grad_array.size(), MPI::FLOAT, 
							&ps_buffer[0], grad_array.size(), MPI::FLOAT, PS);
	
	if ( ClusterID == PS)
	{
		// Copy CPU result to GPU ==========================================
		err = cudaMemcpy(d_grad, &ps_buffer[0], grad_array.size()*sizeof(float), cudaMemcpyHostToDevice);
		assert(err == cudaSuccess);
	...
	// Bcast on CPU ============================================
	MPI::COMM_WORLD.Bcast(&grad_array[0], grad_array.size(), MPI::FLOAT,PS);
	...
	MPI::COMM_WORLD.Barrier();
	std::cout << "Total CPU GatherBcast Time = " << Total_T.Stop() << " seconds. " << std::endl;
